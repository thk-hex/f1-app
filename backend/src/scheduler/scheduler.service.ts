...
export class SchedulerService {
    ...
    try {
      ...
      for (let year = startYear; year <= currentYear; year++) {
        try {
          ...
          this.logger.error(`Failed to update race winners for year ${year}:`, error.message);
          // Continue with other years even if one fails
        }
      }
      
      this.logger.log(`Successfully updated race winners data. Total races: ${totalRacesUpdated}`);
    } catch (error) {
  ...
  getNextScheduledRun(): Date {
    ...
    nextMonday.setUTCDate(now.getUTCDate() + (daysUntilNextMonday === 0 ? 7 : daysUntilNextMonday));
